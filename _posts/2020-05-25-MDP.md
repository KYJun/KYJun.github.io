---
layout: post
title: "강화학습 튜토리얼 03 : 마코프 결정 모델 (MDP)"
categories: rl
---
Markov Decision Process : 마코프 결정 모델
...
앞서 정의한 Agent의 선택 방식이라면, A와 B 모두 전체 Reward의 합이 1000만원이기 때문에 차이가 없는 선택입니다. 

하지만 우리는 지금 받는 백만원과 일년 뒤에 받는 백만원이 같은 가치가 아님을 알고 있습니다. 물가 상승률, 이자율 등등 두 선택에는 명백한 차이가 존재하죠. 그렇다면 기계가 이 차이를 인식하게 하려면 어떻게 해야 할까요?
